function shift_letter_ceasar(letter, shift){//parameter variables are defined when the function is called
...
function code_ceasar(text, shift){
    var codedText = "";
    for (var i=0; i<text.length; i++){//count starts at one, indeces start at 0
      codedText += shift_letter_ceasar(text[i], shift);//text is entire string, letter is single character. you can loop through a string like you would an array.
    }
	return codedText;
}

function decode_ceasar(text, shift){
	return code_ceasar(text, -shift);
}